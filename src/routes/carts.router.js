...
router.post("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Error al agregar carrito" });
  }
});
...
router.post("/:cid/product/:pid", async (req, res) => {
  try {
    ...
    if (!product)
      return res
        .status(404)
        .json({ status: "error", message: "No se encuentra producto" });

    const cart = await Carts.getCart(cid);
    if (!cart)
      return res
        .status(404)
        .json({ status: "error", message: "No se encuentra carrito" });

    const updatedCart = await Carts.postProductCart(cid, pid, Number(quantity));
    if (!updatedCart)
  ...
  try {
    ...
    products.forEach(async (p) => {
      ...
      if (!product)
        return res.status(404).json({
          status: "error",
          message: `No se encuentra producto con id ${p.product}`,
        });
    });
